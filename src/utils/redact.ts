const SECRET_KEY_PATTERN =
  /(authorization|api[_-]?key|token|secret|password|credential|bearer)/i;
...
export function collectSecretValues(...records: Array<Record<string, string> | undefined>): string[] {
...
export function isLikelyCredentialError(text: string): boolean {
  return /(auth|credential|unauthori[sz]ed|forbidden|api key|token|permission|access denied)/i.test(text);
}
